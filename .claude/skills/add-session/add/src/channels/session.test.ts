...
describe('SessionChannel', () => {
  ...
  describe('message handling', () => {
    ...
    it('shows [Attachment: filename] when body is absent and attachment has fileName', async () => {
      const opts = createTestOpts();
      mockState.nextConvo = {
        displayName: 'Alice',
        isApproved: true,
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        REGISTERED_JID,
        expect.objectContaining({ content: '[Attachment: photo.jpg]' }),
      );
    });

    it('shows generic [Attachment] when fileName is missing', async () => {
      const opts = createTestOpts();
      mockState.nextConvo = {
        displayName: 'Alice',
        isApproved: true,
    ...
    it('uses body over attachment when both are present', async () => {
      const opts = createTestOpts();
      mockState.nextConvo = {
        displayName: 'Alice',
        isApproved: true,
